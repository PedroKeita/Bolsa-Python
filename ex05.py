def comprar_frutas(morango=0, uva=0):
    ...
    preço_pago = 0
    kg_fruta = morango + uva

    if morango > 5:
        preço_pago += morango * 2.2
    else:
        preço_pago += morango * 2.5
    ...
    if preço_pago > 25 or kg_fruta > 8:
        return round(preço_pago - (preço_pago * (10 / 100)),2)
    else:
        return round(preço_pago,2)
